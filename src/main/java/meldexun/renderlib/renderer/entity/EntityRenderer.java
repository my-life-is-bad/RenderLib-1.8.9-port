...
public class EntityRenderer {
	...
	public void setup(ICamera frustum, float partialTicks, double camX, double camY, double camZ) {
		...
		this.totalEntities = mc.theWorld.loadedEntityList.size();
		this.camChunkX = MathHelper.floor_double(RenderUtil.getCameraEntityX()) >> 4;
		this.camChunkZ = MathHelper.floor_double(RenderUtil.getCameraEntityZ()) >> 4;
		this.renderDist = mc.gameSettings.renderDistanceChunks;

		EntityRenderList entityList = new EntityRenderList();
		for (Entity entity : EntityUtil.entityIterable(mc.theWorld.loadedEntityList)) {
	...
	private boolean isOutsideOfRenderDist(Entity entity, double partialTicks) {
		int entityX = MathHelper.floor_double(entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * partialTicks) >> 4;
		if (Math.abs(entityX - this.camChunkX) > this.renderDist) {
			return true;
		}
		int entityZ = MathHelper.floor_double(entity.lastTickPosZ + (entity.posZ - entity.lastTickPosZ) * partialTicks) >> 4;
